...
namespace Core.Helper
{
    ...
    public class WebHelper
    {
        ...
        /// <summary>
        /// 获得指定名称的Cookie的expire
        /// </summary>
        /// <param name="name">Cookie名称</param>
        /// <returns></returns>
        public static string GetCookieExpire(string name)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[name];
            if (cookie != null)
            {
                return cookie.Values["expires"];
            }
            return string.Empty;
        }
        ...
        public static string GetRequestData(string url, string method, string postData, Encoding encoding, int timeout)
        {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            using (reader = new StreamReader(response.GetResponseStream(), encoding))
                            {
                                ...
                                {
                                    IOHelper.WriteLogFile("GetRequestData方法，读取异常。地址：" + url + "。提示：" + ex.Message + "");
                                    return "close";
                                }
            ...
            {
                IOHelper.WriteLogFile("GetRequestData方法，请求异常。地址：" + url + "。提示：" + ex.Message + "");
                return "error";
            }
        }
        ...
        public static string PostData(string url, string param)
        {
            ...
            {
                IOHelper.WriteLogFile("接口出错。地址：" + url + "。提示：" + ex.Message + "");
                return "";
            }
        }
